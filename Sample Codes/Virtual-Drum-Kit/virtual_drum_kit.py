...
# ---------------------------------------------------------------------------
# Synthesised drum sounds (generated at import time so they're ready immediately)
# ---------------------------------------------------------------------------
def _make_wav_bytes(audio: np.ndarray, sample_rate: int = 22050) -> io.BytesIO:
    """Convert a float numpy array (-1..1) to an in-memory WAV file."""
...
def _synth_snare(sr: int = 22050) -> io.BytesIO:
...
def _synth_hihat(sr: int = 22050) -> io.BytesIO:
...
def _synth_cymbal(sr: int = 22050) -> io.BytesIO:
...
def _synth_kick(sr: int = 22050) -> io.BytesIO:
    """Low-frequency thump with fast decay (kick drum)."""
    dur = 0.2
    t = np.linspace(0, dur, int(sr * dur), endpoint=False)
    freq = 150 * np.exp(-t * 20) + 50
    tone = np.sin(2 * np.pi * np.cumsum(freq) / sr)
    env = np.exp(-t * 12)
    return _make_wav_bytes(tone * env * 0.9, sr)
...
class DrumPad:
    ...
    synth_func: object                         # callable → io.BytesIO

    # Runtime state (managed by VirtualDrumKit)
    cx: int = 0                                # pixel centre X
    cy: int = 0                                # pixel centre Y
    radius: int = 0                            # pixel radius
    triggered: bool = False                    # finger currently inside?
    cooldown: int = 0                          # remaining cooldown frames
...
class VirtualDrumKit:
    ...
    def _init_pads(self) -> None:
        ...
        definitions = [
            ...
            ("Tom",   (50, 255, 120),  "tom.wav",    _synth_snare),   # reuse snare synth for Tom
    ...
    def _load_sound(self, pad: DrumPad) -> None:
        ...
        if os.path.exists(wav_path):
            pad._sound = self.pygame.mixer.Sound(wav_path)
        else:
            buf = pad.synth_func()
            pad._sound = self.pygame.mixer.Sound(buf)

    # ------------------------------------------------------------------
    # Layout (called once per frame to adapt to resolution changes)
    # ------------------------------------------------------------------
    def _layout_pads(self, h: int, w: int) -> None:
        """Position the drum-pad circles on the frame."""
        n = len(self.pads)
    ...
    def _finger_in_pad(fx: int, fy: int, pad: DrumPad) -> bool:
        """Return True if the point (fx, fy) lies inside *pad*'s circle."""
        dx = fx - pad.cx
        dy = fy - pad.cy
    ...
    def update(self, frame: np.ndarray) -> np.ndarray:
        """
        Run one iteration: detect hands, check collisions, play sounds,
        draw the HUD, and return the annotated frame.
        """
        h, w = frame.shape[:2]
    ...
    def _draw_pads(self, frame: np.ndarray) -> None:
        """Render semi-transparent drum pads with labels."""
        for pad in self.pads:
            # Darker when triggered or on cooldown
            alpha = 0.30
            if pad.triggered:
                alpha = 0.55
            elif pad.cooldown > 0:
                alpha = 0.35 + 0.20 * (1 - pad.cooldown / COOLDOWN_FRAMES)
    ...
    def _draw_skeleton(self, frame: np.ndarray, hand_landmarks_list, h: int, w: int) -> None:
        """Draw MediaPipe hand skeletons (debug mode)."""
        connections = [
            (0, 1), (1, 2), (2, 3), (3, 4),
            (0, 5), (5, 6), (6, 7), (7, 8),
...
# ---------------------------------------------------------------------------
def main() -> None:
    """Run the virtual drum kit."""
    print(f"Model: {MODEL_PATH}")
    print("=" * 55)
    print("Virtual Drum Kit — MediaPipe Hands + Pygame")
